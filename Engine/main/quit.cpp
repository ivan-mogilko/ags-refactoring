    ...
    else if (qmsg[0]=='!')
    {
        ...
        else if (qmsg[0] == '?')
        {
            ...
            error_is = !error_text.IsEmpty() ? error_text :
                "A fatal error reported by the game script. "
        ...
        else
        {
            ...
            error_is = !error_text.IsEmpty() ? error_text :
                "An game error has occurred. Please contact the game author for support, as this is "
                "likely to be an mistake in game logic or script and not a error in AGS engine.";
        }
