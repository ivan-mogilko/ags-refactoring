...
{
    const size_t exp_index = _exportLookup.GetIndexOfAny(fn_name);
...
bool RuntimeScript::ResolveScriptImports(const SystemImports &simp)
{
    ...
    for (size_t import_idx = 0; import_idx < _imports.size(); ++import_idx)
    {
        ...
        resolved_imports[import_idx] = simp.GetIndexOf(_imports[import_idx]);
...
bool RuntimeScript::CreateExports()
{
    ...
    for (size_t i = 0; i < _exports.size(); i++)
    {
        ...
        if (etype == EXPORT_FUNCTION)
        {
            ...
            _resolvedExports[i].Value.SetCodePtr(_code.data()
        ...
        else if (etype == EXPORT_DATA)
        {
            ...
            if (gl_var)
            {
                _resolvedExports[i].Value.SetGlobalVar(&gl_var->RValue);
            }
            else
            {
