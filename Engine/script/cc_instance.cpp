...
bool ccInstance::_Create(PRuntimeScript script, const ccInstance *joined)
{
    ...
    if ((script.use_count() == 1) && (ccGetOption(SCOPT_AUTOIMPORT) != 0))
    {
        // import all the exported stuff from this script
        for (size_t i = 0; i < script->GetExports().size(); i++)
        {
            if (!ccAddExternalScriptSymbol(script->GetExports()[i].GetCStr(), script->GetResolvedExports()[i].Value, script.get()))
            {
                cc_error("Export table overflow at '%s'", script->GetExports()[i].GetCStr());
                return false;
            }
        }
    }
...
{
    // When the base script has no more "instances", remove all script exports
    if (_instanceof.use_count() == 1)
    {
        simp.RemoveScriptExports(_instanceof.get());
    }
    _instanceof = nullptr;
