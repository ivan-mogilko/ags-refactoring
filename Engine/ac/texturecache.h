...
//=============================================================================
//
// TextureCache class stores textures created by the GraphicsDriver from plain
// bitmaps. It consists of two parts:
// * A long-term MRU cache, which keeps texture data even when it's not in
// immediate use, and disposes less used textures to free space when reaching
// the configured mem limit.
// * A short-term cache of texture references, which keeps only weak refs to the
// textures that are currently in use. This short-term cache lets to keep
// reusing same texture so long as there's at least one object on screen that
// uses it.
...
public:
    ...
    TextureCache(Common::SpriteCache *spriteset, IGraphicsDriver *gfx_driver)
        : _spriteset(spriteset), _gfxDriver(gfx_driver) {}
    virtual ~TextureCache() = default;

    void SetSpriteCache(Common::SpriteCache *spriteset) { _spriteset = spriteset; }
    void SetGraphicsDriver(IGraphicsDriver *gfx_driver) { _gfxDriver = gfx_driver; }
    ...
    std::shared_ptr<Texture> GetOrLoad(uint32_t sprite_id, Common::Bitmap *source, bool has_alpha, bool opaque);
...
private:
    ...
    // Graphics driver used to create textures
    ...
    Common::SpriteCache *_spriteset = nullptr;
