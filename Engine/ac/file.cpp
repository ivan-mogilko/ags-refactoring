...
int File_ReadBytes(sc_File *fil, void *arr_obj, int index, int count)
{
    ...
    {
        debug_script_warn("File.ReadBytes: starting index out of bounds: %d, range is %d..%u", index, 0, hdr.GetElemCount());
        return 0;
    }
    if (count < 0)
    {
        debug_script_warn("File.ReadBytes: invalid count: %d", count);
        return 0;
    }
...
int File_WriteBytes(sc_File *fil, void *arr_obj, int index, int count)
{
    ...
    if (index < 0 || static_cast<uint32_t>(index) >= hdr.GetElemCount())
    {
        debug_script_warn("File.WriteBytes: starting index out of bounds: %d, range is %d..%u", index, 0, hdr.GetElemCount());
        return 0;
    }
    if (count < 0)
    {
        debug_script_warn("File.WriteBytes: invalid count: %d", count);
        return 0;
    }
